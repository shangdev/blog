---
title: WordPress 优化指南
date: 2018-12-23 14:46:19
tags:
- wordpress
- 优化
categories:
...
这篇我们从服务器层面优化 WordPress 的加载速度。推荐使用 PHP7.2 及以上，PHP7 的几个版本 Benchmark 的测试 [见这里](https://kinsta.com/blog/php-benchmarks/)，也就是版本越高性能越好，WordPress 完全兼容 PHP7。

- 启用 Zend Opcache
- 开启 Opcache 的 HugePage
- 启用 Memcached
- 其他
...
Memcached 默认端口为 11211，不建议修改

启用 Memcached 服务：

```linux
$ sudo chkconfig --level 2345 memcached on //开机启动
...
接下来安装 WordPress 对象缓存插件，将以下文件（命名为object-cache.php）内容复制到 wp-content 目录下即可：
```php
<?php
...
推荐 GCC4.8 以上，所以直接用 Centos Latest 吧，Ubuntu 也是

## Opcache file cache（试验性）
...
详细设置见鸟哥的这篇文章 [让你的PHP7更快(GCC PGO) ](http://www.laruence.com/2015/06/19/3063.html)

以 WordPress 5.0.2 为优化场景，在编译 PHP 的时候首先：

`$ make prof-gen`

然后使用 WordPress 来训练 PHP：

```linux
$ sapi/cgi/php-cgi -T 100
/path_to_wordpress/index.php > /dev/null
```

即让 php-cgi 跑100遍 WordPress 的首页, 从而生成一些在这个过程中的 profile 信息
...
这个时候的 PHP7 就是为 WordPress 量身打造的最高性能的编译版本